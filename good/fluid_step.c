...
double* advection(double t, double *array, double k) {
	...
	for (int i=0; i<SIM_SIZE; i++) {
		for (int j=0; j<SIM_SIZE; j++) {
			double avg[5]={0, 0, 0, 0, 0};
			double div=4;
			double to[]={j+array[(i*SIM_SIZE+j)*5]*t, i+array[(i*SIM_SIZE+j)*5+1]*t};
			if (to[0]>0 and to[0]<SIM_SIZE-1 and to[1]>0 and to[1]<SIM_SIZE-1) {
				double dxA=to[0]-(int)to[0];
				double dyA=to[1]-(int)to[1];
				double dA=sqrt(dxA*dxA+dyA*dyA);
				double dxB=(int)(to[0]+1)-to[0];
				double dyB=dyB-(int)to[1];
				double dB=sqrt(dxB*dxB+dyB*dyB);
				double dxC=dxA;
				double dyC=(int)(to[1]+1)-to[1];
				double dC=sqrt(dxC*dxC+dyC*dyC);
				double dxD=dxB;
				double dyD=dyC;
				double dD=sqrt(dxD*dxD+dyD*dyD);
				double tot=dA+dB+dC+dD;
				move(array+5*(i*SIM_SIZE+j), n_arr+(((int)(to[1]))*SIM_SIZE+(int)(to[0]))*5, (dB+dC+dD)/(3.0*tot));
				move(array+5*(i*SIM_SIZE+j), n_arr+(((int)(to[1])+1)*SIM_SIZE+(int)(to[0]))*5, (dA+dC+dD)/(3.0*tot));
				move(array+5*(i*SIM_SIZE+j), n_arr+(((int)(to[1]))*SIM_SIZE+(int)(to[0]+1))*5, (dA+dB+dD)/(3.0*tot));
				move(array+5*(i*SIM_SIZE+j), n_arr+(((int)(to[1]+1))*SIM_SIZE+(int)(to[0]+1))*5, (dA+dC+dB)/(3.0*tot));
	...
	for (int k=0; k<7; k++) {
		...
		for (int i=0; i<SIM_SIZE; i++) {
			for (int j=0; j<SIM_SIZE; j++) {
				...
				if (k==6 and i>0 and i<SIM_SIZE-1 and j>0 and j<SIM_SIZE-1) {
...
double *sub_tensor(double *arg1, double *arg2) {
	...
	for (int i=0; i<SIM_SIZE*SIM_SIZE; i++) {
		...
		double diff=1-n_arr[i*5+3]/maxP;
		n_arr[i*5]-=arg2[i*5]*diff; //what this effectively means is that a part of the field with 0 density takes out 100% of its convergence/divergence
		//but a part of the field with maximum density gets to go as it pleases (able to move in convergence and divergence)
		//a part of the field with negative density overcompensates.
		n_arr[i*5+1]-=arg2[i*5+1]*diff;
		n_arr[i*5+2]-=arg2[i*5+1]*diff;
...
void gravity(double x1, double y1, double z1, double x2, double y2,double z2) {
	double *n_arr=fluid_step_mem;
	//double v1[]={x2-x1, y2-y1, z2-z1};
	for (int i=0; i<SIM_SIZE; i++) {
		for (int j=0; j<SIM_SIZE; j++) {
			if (1) {
				double dist=(fabs((j-x2)*(j-x2)+(i-y2)*(i-y2))+.1);
				if (n_arr[(i*SIM_SIZE+j)*5+3]>0.00000001) {
					dist/=n_arr[(i*SIM_SIZE+j)*5+3]; //this is the same as multiplying by density. 
					//the idea isn't so much to have heavier things fall faster, but to have heavier things overcome the no-convergence
					//conditions on movement
				}
				double v[]={(x2-j)/dist, (y2-i)/dist, v[2]/dist};
			
					n_arr[(i*SIM_SIZE+j)*5]+=v[0];
					n_arr[(i*SIM_SIZE+j)*5+1]+=v[1];
				
					n_arr[(i*SIM_SIZE+j)*5+2]+=v[2];
	...
	double *adv=advection(.01, n_arr, .01);
	memcpy(n_arr, adv, SIM_SIZE*SIM_SIZE*5*sizeof(double));
}
//this magnetic function seems to be causing a problem of wiping everything off the map.
double magnetic_mem[SIM_TOT];
void magnetic(double x1, double y1, double z1, double x2, double y2,double z2) {
	...
	for (int i=0; i<SIM_SIZE; i++) {
		for (int j=0; j<SIM_SIZE; j++) {
			double dist=fabs((j-x1)*(j-x1)*(j-x2)*(j-x2)+(i-y1)*(i-y1)*(i-y2)*(i-y2))+.1; //the +.1 prevents information loss at the singularity.
