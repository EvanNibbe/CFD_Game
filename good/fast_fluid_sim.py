...
while running:
	...
	if (max(max(new_mouse), max(prev_mouse))<2*SIM_SIZE) and (min(min(new_mouse), min(prev_mouse))>0):
		...
		if pygame.mouse.get_pressed()[0]:
			fluid_sim[new_mouse[1]//2][new_mouse[0]//2][3]+=sqrt(xV**2+yV**2)/10.0
			fluid_sim[new_mouse[1]//2][new_mouse[0]//2][4]+=sqrt(xV**2+yV**2)/10.0
			fluid_sim[prev_mouse[1]//2][prev_mouse[0]//2][3]+=sqrt(xV**2+yV**2)/10.0
			fluid_sim[prev_mouse[1]//2][prev_mouse[0]//2][4]+=sqrt(xV**2+yV**2)/10.0
		#the magnetic function has the problem of wiping everything off the map
		#the gravity function also has the problem of wiping everything off the map.
		if pygame.mouse.get_pressed()[1]:
			gravity(c_double(prev_mouse[0]//2), c_double(prev_mouse[1]//2), c_double(1), c_double(new_mouse[0]//2), c_double(new_mouse[1]//2), c_double(1))
		if pygame.mouse.get_pressed()[2]:
			magnetic(c_double(prev_mouse[0]//2), c_double(prev_mouse[1]//2), c_double(-1), c_double(new_mouse[0]//2), c_double(new_mouse[1]//2), c_double(1))
